// Copyright (c) 2015 The Okcash developers
        ...
        if (eKey58.Set58(sInKey.c_str()) == 0)
        {
            if (fBip44)
            {
                if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY_BTC))
                    throw std::runtime_error("Import failed - BIP44 key must begin with Okcash secret key prefix.");
            } else
            {
                if (!eKey58.IsValid(CChainParams::EXT_SECRET_KEY)
                    && !eKey58.IsValid(CChainParams::EXT_PUBLIC_KEY_BTC))
                    throw std::runtime_error("Import failed - Key must begin with Okcash prefix.");
            };

...
function getCostMatrixMemorySizeMB(
  ...
  const costMatrixMemorySizeMB =
    (sequence1Length * Math.min(sequence2Length, windowLength) * 4) / 1000000;
...
function varianceOfVectors(
  ...
  const sampleSizeMetric =
    kind == "population" || vectorCount == 1 ? vectorCount : vectorCount - 1;
  const featureCount = vectors[0].length;
...
function applyLiftering(mfccs: number[][], lifteringFactor: number) {
  ...
  for (let i = 0; i < featureCount; i++) {
    lifterMultipliers[i] =
      1 +
      (lifteringFactor / 2) * Math.sin((Math.PI * (i + 1)) / lifteringFactor);
  }
...
function getMfccOptionsForGranularity(granularity: DtwGranularity) {
  ...
  if (granularity == "xx-low") {
    mfccOptions = { windowDuration: 0.4, hopDuration: 0.16, fftOrder: 8192 };
  } else if (granularity == "x-low") {
    mfccOptions = { windowDuration: 0.2, hopDuration: 0.08, fftOrder: 4096 };
  } else if (granularity == "low") {
    mfccOptions = { windowDuration: 0.1, hopDuration: 0.04, fftOrder: 2048 };
  } else if (granularity == "medium") {
    mfccOptions = { windowDuration: 0.05, hopDuration: 0.02, fftOrder: 1024 };
  } else if (granularity == "high") {
    mfccOptions = { windowDuration: 0.025, hopDuration: 0.01, fftOrder: 512 };
  } else if (granularity == "x-high") {
    mfccOptions = { windowDuration: 0.02, hopDuration: 0.005, fftOrder: 512 };
  } else {
...
async function synthesizeFragments(
  ...
  for (const [index, wordEntry] of wordTimeline.entries()) {
    ...
    const previousWordTokenIPAWithoutStress = previousWordTokenEntry.timeline
      .map((phoneEntry) =>
    ...
    if (
      previousWordTokenEntry.timeline.length <= wordReferencePhonemes.length
    ) {
      continue;
    }

    if (
      !previousWordTokenIPAWithoutStress.endsWith(wordReferenceIPAWithoutStress)
    ) {
      continue;
    }
...
class VitsTTS {
  ...
  private async _synthesizeSentence(
    ...
    const scalesTensor = new Onnx.Tensor(
      "float32",
      [metadata.inference.noise_scale, lengthScale, metadata.inference.noise_w],
      [3],
    );
...
async function synthesizeSegments(
  ...
  for (let segmentIndex = 0; segmentIndex < segments.length; segmentIndex++) {
    ...
    for (
      ...
      const { synthesizedAudio: sentenceRawAudio, timeline: sentenceTimeline } =
        await synthesizeSegment(sentenceText, sentencetSynthesisOptions);
      ...
      const sentenceAudioLength =
        sentenceRawAudio.audioChannels[0].length / sentenceRawAudio.sampleRate;
    ...
    segmentEntry.endTime =
      segmentEntry.timeline?.[segmentEntry.timeline.length - 1]?.endTime ||
      timeOffset;
...
async function synthesizeSegment(text: string, options: SynthesisOptions) {
  ...
  if (timeline) {
    ...
    const newDuration =
      synthesizedAudio.audioChannels[0].length / synthesizedAudio.sampleRate;
...
async function downloadFile(
  ...
  function updateStatus() {
    ...
    const windowStartRecord = downloadStateHistory.find(
      (r) => r.time >= timer.elapsedTimeSeconds - rateAveragingWindowSeconds,
    );
...
function mapUsingUncropTimeline(
  ...
  for (const uncropEntry of uncropTimeline) {
    ...
    if (overlapDuration >= 0 && overlapDuration > bestOverlapDuration) {
      ...
      mappedStartTime =
        uncropEntry.startTime + (overlapStartTime - offsetInCroppedAudio);
      mappedEndTime =
...
async function requestVoiceList(
  ...
  if (options.voiceGender) {
    ...
    voiceList = voiceList.filter(
      (voice) => voice.gender == genderLowercase || voice.gender == "unknown",
    );
  }

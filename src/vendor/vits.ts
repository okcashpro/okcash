...
function getCostMatrixMemorySizeMB(sequence1Length: number, sequence2Length: number, windowLength: number) {
...
function alignDTWWindowed<T, U>(sequence1: T[], sequence2: U[], costFunction: (a: T, b: U) => number, windowMaxLength: number, centerIndexes?: number[]) {
...
async function alignMFCC_DTW(mfccFrames1: number[][], mfccFrames2: number[][], windowLength: number, distanceFunctionKind: 'euclidian' | 'cosine' = 'euclidian', centerIndexes?: number[]) {
...
async function stftr(samples: Float32Array, fftOrder: number, windowSize: number, hopSize: number, windowType: WindowType) {
...
async function computeMelSpectogramUsingFilterbanks(rawAudio: RawAudio, fftOrder: number, windowSize: number, hopLength: number, filterbanks: Filterbank[], windowType: WindowType = 'hann') {
	const logger = new Logger()

	logger.start('Compute short-time FFTs')
	const audioSamples = rawAudio.audioChannels[0]
	const fftFrames = await stftr(audioSamples, fftOrder, windowSize, hopLength, windowType)
...
function fftFramesToMelSpectogram(fftFrames: Float32Array[], melFilterbanks: Filterbank[]) {
...
function powerSpectrumToMelSpectrum(powerSpectrum: Float32Array, filterbanks: Filterbank[]) {
...
function getMelFilterbanks(powerSpectrumFrequenciesHz: Float32Array, centerFrequenciesMel: Float32Array, lowerFrequencyMel: number, upperFrequencyMel: number) {
...
function getMelFilterbanksCenterFrequencies(melBandCount: number, lowerFrequencyMel: number, upperFrequencyMel: number) {
...
async function computeMelSpectogram(rawAudio: RawAudio, fftOrder: number, windowSize: number, hopLength: number, filterbankCount: number, lowerFrequencyHz: number, upperFrequencyHz: number, windowType: WindowType = 'hann') {
	const logger = new Logger()
...
function melSpectrumToMFCC(melSpectrum: Float32Array, mfccFeatureCount: number, dctMatrix: Float32Array[], normalization: 'none' | 'orthonormal' = 'orthonormal') {
...
function createDCTType2CoefficientMatrix(mfccFeatureCount: number, melBandCount: number) {
...
function melSpectogramToMFCCs(melSpectogram: Float32Array[], mfccFeatureCount: number) {
...
function applyEmphasis(samples: Float32Array, emphasisFactor = 0.97, initialState = 0) {
...
async function resampleAudioSpeex(rawAudio: RawAudio, outSampleRate: number, quality = 0): Promise<RawAudio> {
...
function normalizeVectors(vectors: number[][], kind: 'population' | 'sample' = 'population') {
...
function stdDeviationOfVectors(vectors: number[][], kind: 'population' | 'sample' = 'population', mean?: number[]) {
	return varianceOfVectors(vectors, kind, mean).map(v => Math.sqrt(v))
}

function varianceOfVectors(vectors: number[][], kind: 'population' | 'sample' = 'population', mean?: number[]) {
	const vectorCount = vectors.length
...
function euclidianDistance(vector1: ArrayLike<number>, vector2: ArrayLike<number>) {
	return Math.sqrt(squaredEuclidianDistance(vector1, vector2))
}

function squaredEuclidianDistance(vector1: ArrayLike<number>, vector2: ArrayLike<number>) {
...
function euclidianDistance13Dim(vector1: ArrayLike<number>, vector2: ArrayLike<number>) {
	return Math.sqrt(squaredEuclidianDistance13Dim(vector1, vector2))
}

function squaredEuclidianDistance13Dim(vector1: ArrayLike<number>, vector2: ArrayLike<number>) {
...
function cosineDistancePrecomputedMagnitudes(vector1: ArrayLike<number>, vector2: ArrayLike<number>, magnitude1: number, magnitude2: number) {
	return 1 - cosineSimilarityPrecomputedMagnitudes(vector1, vector2, magnitude1, magnitude2)
}

function cosineSimilarityPrecomputedMagnitudes(vector1: ArrayLike<number>, vector2: ArrayLike<number>, magnitude1: number, magnitude2: number) {
	if (vector1.length != vector2.length) {
		throw new Error('Vectors are not the same length')
	}
...
function getStartingSilentSampleCount(audioSamples: Float32Array, amplitudeThresholdDecibels = defaultSilenceThresholdDecibels) {
...
function getEndingSilentSampleCount(audioSamples: Float32Array, amplitudeThresholdDecibels = defaultSilenceThresholdDecibels) {
	const minSampleAmplitude = decibelsToGainFactor(amplitudeThresholdDecibels)
...
async function alignUsingDtw(
	...
	for (let passIndex = 0; passIndex < windowDurations.length; passIndex++) {
		...
		logger.logTitledMessage(`\nStarting alignment pass ${passIndex + 1}/${windowDurations.length}`, `granularity: ${granularity}, max window duration: ${windowDuration}s`)
		...
		if (passIndex == 0) {
			...
			if (windowDuration < minRecommendedWindowDuration) {
				logger.logTitledMessage('Warning', `Maximum DTW window duration is set to ${windowDuration.toFixed(1)}s, which is smaller than 20% of the source audio duration of ${rawAudioDuration.toFixed(1)}s. This may lead to suboptimal results in some cases. Consider increasing window duration if needed.`, 'warning')
			}
		}
...
async function splitChineseTextToWords_Jieba(text: string, fineGrained = false, useHMM = true) {
...
function tryGetFirstLexiconSubstitution(sentenceWords: string[], wordIndex: number, lexicons: Lexicon[], languageCode: string) {
...
function tryGetLexiconSubstitution(sentenceWords: string[], wordIndex: number, lexicon: Lexicon, languageCode: string) {
...
type EspeakEventType = 'sentence' | 'word' | 'phoneme' | 'end' | 'mark' | 'play' | 'msg_terminated' | 'list_terminated' | 'samplerate'
...
async function synthesizeFragments(fragments: string[], espeakOptions: EspeakOptions) {
	...
	const { rawAudio, events } = await espeakSynthesize(textWithMarkers, { ...espeakOptions, ssml: true })
...
async function preprocessAndSynthesize(text: string, language: string, espeakOptions: EspeakOptions, lexicons: Lexicon[] = []) {
	const logger = new Logger()
...
type LexiconWordCase = 'any' | 'capitalized' | 'uppercase' | 'lowercase' | 'titlecase' | 'camelcase' | 'pascalcase'
...
class VitsTTS {
	...
	static async synthesizeSentence(
		...
		const result = await vitsTTS._synthesizeSentence(text, lengthScale, speakerId, lexicons)

		return result
	}

	private async _synthesizeSentence(sentence: string, lengthScale: number, speakerId = 0, lexicons?: Lexicon[]) {
		const logger = new Logger()
		...
		const { referenceSynthesizedAudio, referenceTimeline, fragments, phonemizedFragmentsSubstitutions, phonemizedSentence } = await preprocessAndSynthesize(sentence, languageCode, espeakOptions, lexicons)
...
async function detectTextLanguage(input: string, options: TextLanguageDetectionOptions): Promise<TextLanguageDetectionResult> {
	const logger = new Logger()
...
function encodeToAudioBuffer(audioChannels: Float32Array[], targetBitDepth: BitDepth = 16, targetSampleFormat: SampleFormat = SampleFormat.PCM) {
...
function encodeWave(rawAudio: RawAudio, bitDepth: BitDepth = 16, sampleFormat: SampleFormat = SampleFormat.PCM, speakerPositionMask = 0) {
	const audioChannels = rawAudio.audioChannels
	const sampleRate = rawAudio.sampleRate

	const audioBuffer = encodeToAudioBuffer(audioChannels, bitDepth, sampleFormat)
...
function encodeRawAudioToWave(rawAudio: RawAudio, bitDepth: BitDepth = 16, sampleFormat: SampleFormat = SampleFormat.PCM, speakerPositionMask = 0) {
...
function trimAudioStart(audioSamples: Float32Array, targetStartSilentSampleCount = 0, amplitudeThresholdDecibels = defaultSilenceThresholdDecibels) {
...
function trimAudioEnd(audioSamples: Float32Array, targetEndSilentSampleCount = 0, amplitudeThresholdDecibels = defaultSilenceThresholdDecibels) {
...
async function synthesizeSegments(segments: string[], options: SynthesisOptions, onSegment?: SynthesisSegmentEvent, onSentence?: SynthesisSegmentEvent): Promise<SynthesisResult> {
	...
	if (!options.language && !options.voice) {
		...
		const { detectedLanguage } = await detectTextLanguage(segmentsPlainText.join('\n\n'), options.languageDetection || {})
	...
	async function convertToTargetCodecIfNeeded(rawAudio: RawAudio) {
		...
		if (targetCodec) {
			...
			if (targetCodec == 'wav') {
				...
				const ffmpegOptions = getDefaultFFMpegOptionsForSpeech(targetCodec, options.outputAudioFormat?.bitrate)
...
async function encodeFromChannels(rawAudio: RawAudio, outputOptions: FFMpegOutputOptions) {
	return transcode(encodeRawAudioToWave(rawAudio), outputOptions)
}

function getDefaultFFMpegOptionsForSpeech(fileExtension: string, customBitrate?: number) {
...
async function synthesizeSegment(text: string, options: SynthesisOptions) {
	...
	switch (engine) {
		...
		case 'espeak': {
			...
			if (inputIsSSML) {
				...
				const { rawAudio } = await espeakSynthesize(text, espeakOptions)
				...
				const { referenceSynthesizedAudio, referenceTimeline } = await preprocessAndSynthesize(text, espeakLanguage, espeakOptions, lexicons)
	...
	if (!timeline) {
		...
		const { wordTimeline } = await align(synthesizedAudio, plainText, alignmentOptions)
...
async function ensureRawAudio(input: AudioSourceParam, outSampleRate?: number, outChannelCount?: number) {
	...
	if (inputAsRawAudio.audioChannels?.length > 0 && inputAsRawAudio.sampleRate) {
		...
		inputAsRawAudio = await decodeToChannels(inputAsStringOrBuffer, outSampleRate, outChannelCount)
	} else {
...
function resolveVersionTagForUnversionedPackageName(unversionedPackageName: string) {
...
async function downloadFile(options: GaxiosOptions, targetFilePath: string, prompt = 'Downloading') {
...
async function downloadAndExtractTarball(options: GaxiosOptions, targetDir: string, baseTempPath: string, displayName = 'archive') {
	const logger = new Logger()
...
function getGlobalOption<K extends keyof GlobalOptions>(key: K): GlobalOptions[K] {
...
async function transcode(input: string | Buffer, outputOptions: FFMpegOutputOptions) {
...
function decodeWave(waveData: Buffer, ignoreTruncatedChunks = true, ignoreOverflowingDataChunks = true) {
	let readOffset = 0
	...
	const audioChannels = decodeAudioBufferToChannels(concatenatedDataBuffers, channelCount, bitDepth, sampleFormat)
...
function decodeAudioBufferToChannels(audioBuffer: Buffer, channelCount: number, sourceBitDepth: number, sourceSampleFormat: SampleFormat) {
	...
	if (sourceSampleFormat === SampleFormat.PCM) {
		if (sourceBitDepth === 8) {
			...
			interleavedChannels = int16PcmToFloat32(bufferLEToInt16(audioBuffer))
		} else if (sourceBitDepth === 24) {
			interleavedChannels = int24PcmToFloat32(bufferLEToInt24(audioBuffer))
		} else if (sourceBitDepth === 32) {
			interleavedChannels = int32PcmToFloat32(bufferLEToInt32(audioBuffer))
		} else {
		...
		if (sourceBitDepth === 32) {
			...
			interleavedChannels = float64Tofloat32(bufferLEToFloat64(audioBuffer))
		} else {
...
function deInterleaveChannels(interleavedChannels: Float32Array, channelCount: number) {
	if (channelCount === 0) {
...
function decodeWaveToRawAudio(waveFileBuffer: Buffer, ignoreTruncatedChunks = true, ignoreOverflowingDataChunks = true) {
	return decodeWave(waveFileBuffer, ignoreTruncatedChunks, ignoreOverflowingDataChunks)
}

async function decodeToChannels(input: string | Buffer, outSampleRate?: number, outChannelCount?: number) {
...
async function align(input: AudioSourceParam, transcript: string, options: AlignmentOptions): Promise<AlignmentResult> {
	const logger = new Logger()
	...
	if (options.language) {
		...
		const { detectedLanguage } = await detectTextLanguage(transcript, options.languageDetection || {})
...
async function wordTimelineToSegmentSentenceTimeline(wordTimeline: Timeline, transcript: string, language: string, paragraphBreaks: ParagraphBreakType = 'double', whitespace: WhitespaceProcessing = 'collapse') {
...
function convertCroppedToUncroppedTimeline(timeline: Timeline, uncropTimeline: Timeline) {
...
function addWordTextOffsetsToTimeline(timeline: Timeline, text: string, currentOffset = 0) {
...
function downmixToMonoAndNormalize(rawAudio: RawAudio, targetPeakDecibels = -3) {
...
async function loadLexiconsForLanguage(language: string, customLexiconPaths?: string[]) {
...
function normalizeAudioLevel(rawAudio: RawAudio, targetPeakDecibels = -3, maxGainIncreaseDecibels = 30): RawAudio {
...
async function writeFile(filePath: string, data: string | NodeJS.ArrayBufferView, options?: fsExtra.WriteFileOptions) {
...
async function writeFileSafe(filePath: string, data: string | NodeJS.ArrayBufferView, options?: fsExtra.WriteFileOptions) {
...
// Voice list request
/////////////////////////////////////////////////////////////////////////////////////////////
async function requestVoiceList(options: VoiceListRequestOptions): Promise<RequestVoiceListResult> {
...
function splitToParagraphs(text: string, paragraphBreaks: ParagraphBreakType, whitespace: WhitespaceProcessing) {

...
import { Action, ActionExample, Content, Message, State, UUID } from "../core/types.ts";
import { ContentBlock } from "@anthropic-ai/sdk/resources/messages";

export const claudeHandlerTemplate =
  `# Attachments
{{attachments}}
...
export default {
  ...
  handler: async (runtime: AgentRuntime, message: Message, state: State, options: any, callback: any) => {
    ...
    [
        {
          user: "{{user1}}",
          content: {
            content: "I need to write a compelling cover letter for a job application as a marketing manager at a tech startup. Can you help me structure it and highlight key points?",
            action: "WAIT",
          },
        },
        {
      ...
      [
        {
          user: "{{user1}}",
          content: {
            content: "I'm planning a trip to Japan next month and I'm overwhelmed with all the information. Can you help me create a 10-day itinerary that covers Tokyo, Kyoto, and Osaka, including must-see attractions, local cuisine recommendations, and transportation tips?",
            action: "WAIT",
          },
        },
        {
          user: "{{agentName}}",
          content: {
            content: "No problem. I'll ask Claude to help create a detailed itinerary for you. Just a moment while I gather that information.",
            action: "ASK_CLAUDE",
          },
        },
      ],

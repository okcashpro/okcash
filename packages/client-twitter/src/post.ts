...
import { postActionResponseFooter } from "@okcashpro/okai";
import { generateTweetActions } from "@okcashpro/okai";
import { IImageDescriptionService, ServiceType } from "@okcashpro/okai";
...
export class TwitterPostClient {
    ...
    async start(postImmediately: boolean = false) {
        ...
        const processActionsLoop = async () => {
            ...
            while (!this.stopProcessingActions) {
                try {
                    ...
                    if (results) {
                        okaiLogger.log(`Processed ${results.length} tweets`);
                        okaiLogger.log(`Next action processing scheduled in ${actionInterval / 1000} seconds`);
                        // Wait for the full interval before next processing
                        await new Promise(resolve => setTimeout(resolve, actionInterval));
                    }
                } catch (error) {
                    okaiLogger.error("Error in action processing loop:", error);
        ...
        if (enableActionProcessing) {
            processActionsLoop().catch(error => {
                okaiLogger.error("Fatal error in process actions loop:", error);
            });
        } else {
            okaiLogger.log("Action processing loop disabled by configuration");
        }
        generateNewTweetLoop();
    }
    ...
    private async generateNewTweet() {
        ...
        try {
            ...
            if (!cleanedContent) {
                okaiLogger.error('Failed to extract valid content from response:', {
    ...
    private async generateTweetContent(tweetState: any, options?: {
        ...
        try {
            ...
            okaiLogger.debug('Response is not JSON, treating as plain text');
        }
    ...
    private async processTweetActions() {
        if (this.isProcessing) {
            okaiLogger.log('Already processing tweet actions, skipping');
            return null;
        }
        ...
        try {
            ...
            okaiLogger.log("Processing tweet actions");
            ...
            for (const tweet of homeTimeline) {
                try {
                    ...
                    if (memory) {
                        okaiLogger.log(`Already processed tweet ID: ${tweet.id}`);
                        continue;
                    }
                    ...
                    if (!actionResponse) {
                        okaiLogger.log(`No valid actions generated for tweet ${tweet.id}`);
                        continue;
                    }
                    ...
                    if (actionResponse.like) {
                        try {
                            ...
                            okaiLogger.log(`Liked tweet ${tweet.id}`);
                        } catch (error) {
                            okaiLogger.error(`Error liking tweet ${tweet.id}:`, error);
                        }
                    }
                    ...
                    if (actionResponse.retweet) {
                        try {
                            ...
                            okaiLogger.log(`Retweeted tweet ${tweet.id}`);
                        } catch (error) {
                            okaiLogger.error(`Error retweeting tweet ${tweet.id}:`, error);
                        }
                    }
                    ...
                    if (actionResponse.quote) {
                        try {
                            ...
                            if (tweet.photos?.length > 0) {
                                okaiLogger.log('Processing images in tweet for context');
                            ...
                            if (tweet.quotedStatusId) {
                                try {
                                    ...
                                    okaiLogger.error('Error fetching quoted tweet:', error);
                                }
                            }
                            ...
                            if (!quoteContent) {
                                okaiLogger.error('Failed to generate valid quote tweet content');
                                return;
                            }

                            okaiLogger.log('Generated quote tweet content:', quoteContent);
                            ...
                            if (body?.data?.create_tweet?.tweet_results?.result) {
                                okaiLogger.log('Successfully posted quote tweet');
                                ...
                                okaiLogger.error('Quote tweet creation failed:', body);
                            }
                        } catch (error) {
                            okaiLogger.error('Error in quote tweet generation:', error);
                        }
                    }
                    ...
                    if (actionResponse.reply) {
                        try {
                            ...
                            okaiLogger.error(`Error replying to tweet ${tweet.id}:`, error);
                        }
                    }
                    ...
                    okaiLogger.error(`Error processing tweet ${tweet.id}:`, error);
                    continue;
                }
            }
            ...
            okaiLogger.error('Error in processTweetActions:', error);
            throw error;
        } finally {
    ...
    private async handleTextOnlyReply(tweet: Tweet, tweetState: any, executedActions: string[]) {
        try {
            ...
            if (!replyText) {
                okaiLogger.error('Failed to generate valid reply content');
                return;
            }

            okaiLogger.debug('Final reply text to be sent:', replyText);
            ...
            if (body?.data?.create_tweet?.tweet_results?.result) {
                okaiLogger.log('Successfully posted reply tweet');
                ...
                okaiLogger.error('Tweet reply creation failed:', body);
            }
        } catch (error) {
            okaiLogger.error('Error in handleTextOnlyReply:', error);
        }
    }

...
export class SlackUtils {
  ...
  static async sendMessageWithRetry(
    ...
    throw new Error(`Failed to send message after ${finalRetryOpts.maxRetries} attempts: ${lastError?.message}`);
  }

  /**
   * Validates if a channel exists and is accessible
   */
  static async validateChannel(client: WebClient, channelId: string): Promise<boolean> {
    try {
   ...
   */
  static formatMessage(text: string, options?: {
    blocks?: any[];
    attachments?: any[];
  }) {
    return {
  ...
  static async withRateLimit<T>(
    ...
    throw new Error(`Operation failed after ${retryOpts.maxRetries} attempts: ${lastError?.message}`);
  }
} 
...
const mockAuthTest = jest.fn<() => Promise<AuthTestResponse>>().mockResolvedValue({ 
  ok: true,
...
const mockPostMessage = jest.fn<() => Promise<ChatPostMessageResponse>>().mockResolvedValue({
  ok: true,
  channel: 'C123456',
...
const mockConversationsInfo = jest.fn<() => Promise<ConversationsInfoResponse>>().mockResolvedValue({
  ok: true,
  channel: {
    id: 'C123456',
    name: 'test-channel',
...
const mockFilesUpload = jest.fn<() => Promise<FilesUploadResponse>>().mockResolvedValue({
  ok: true,
  file: {
    id: 'F123456',
    name: 'test-file',
    ...
    url_private_download: 'https://test.slack.com/files/test-file/download',
...
const mockFilesUploadV2 = jest.fn<() => Promise<FilesUploadResponse>>().mockResolvedValue({
  ok: true,
  file: {
    id: 'F123456',
    created: 12345678,
...
export function simulateRateLimit(client: Mocked<WebClient>) {
  const mockPostMessage = client.chat.postMessage as Mocked<typeof client.chat.postMessage>;
